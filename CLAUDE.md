...
8. **`Agent(retry_config=...)` を必ず渡す**。渡さないと ADK はリトライを一切
   行わず、Gemini の一時的な `503 UNAVAILABLE`（混雑）がそのままユーザーの失敗に
   なる。かつ **ADK はリトライする場合も先にエラーイベントを流す**ので、
   `error_code` 付きイベントを見た瞬間に例外化すると、回復するはずの実行を潰す。
   エラーは保持だけして、**ストリーム終了時に「テキストが 1 つも出ず、かつエラーを
   見た」ときだけ**例外にする。

### 5. LLM の失敗はユーザーに見える言葉へ翻訳する

プロバイダが返す生のペイロード（`{'error': {'code': 503, ...}}`）を画面に出さない。
エージェント層は機械可読な `code` を持つ例外を投げ、**API 層が
`app/utils/errors.py` の純粋関数でユーザー向けの文言に変換**する
（混雑 / レート制限 / 設定ミス / その他）。詳細はサーバのログに残す。

分類の目的は文面を整えることではなく、**クライアントが「再送を勧めてよいか」を
判断できるようにする**こと。混雑とレート制限は再送で直るが、モデル名の誤りや
権限不足は何度送っても直らない。

### 6. フロントエンドは「フック = 状態、コンポーネント = 表示」
